...
const EditQuestion = () => {
    ...
    useEffect(() => {
        const fetchQuestion = async () => {
            try {
                ...
                const { questionText, options, answer, quiz } = response.data;
                setFormData({ questionText, options, answer, quiz});
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
                  // Vérifier si la réponse se trouve dans la liste des options
    if (!formData.options.includes(formData.answer)) {
        setAlertMessage({ type: 'danger', message: 'The answer must be one of the options!' });
        return;
    }
            const response = await axios.put(`http://localhost:5000/question/${id}`, formData, {
                headers: {
                    'Authorization': `Bearer ${token}`,
            ...
            setAlertMessage({ type: 'success', message: 'Question updated successfully!' });

            setTimeout(() => {
                history.push(`/${formData.quiz}/quiz-questions`);
            }, 1000);
        } catch (error) {
            console.error('Error updating question:', error);
            setAlertMessage({ type: 'danger', message: 'Failed to update question!' });
        } finally {
            setTimeout(() => {
    ...
    return (
        <>
            <div className="d-flex align-items-center mb-4">
                <h4 className="fs-20 font-w600 mb-0 me-auto">Edit Question</h4>
            </div>
            {alertMessage && (
                        ...
                        <div className="card-body">
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="card-footer text-end">
                                    <div>
                                        <Link to={`/${formData.quiz}/quiz-questions`} className="btn btn-primary me-3">Cancel</Link>
